...
const Subproject: React.FC<SubprojectProps> = ({ theme }) => {
  ...
  return (
    <div className="space-y-6">
      {/* Header: 1. Icon + Heading, 2. Description */}
      <div className="flex flex-col gap-2 mb-2">
        <div className="flex items-center justify-center sm:justify-start gap-3">
          <div className={`p-2.5 sm:p-3 rounded-xl flex-shrink-0 ${isDark ? 'bg-[#C2D642]/10' : 'bg-[#C2D642]/5'}`}>
            <Layers className="w-5 h-5 sm:w-6 sm:h-6 text-[#C2D642]" />
          </div>
          <h1 className={`text-xl sm:text-2xl font-black tracking-tight ${textPrimary}`}>Subproject</h1>
        </div>
