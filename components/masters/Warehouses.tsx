...
const Warehouses: React.FC<WarehousesProps> = ({ theme }) => {
  ...
  const handleDeleteWarehouse = async (warehouse: WarehouseData | null) => {
    if (!warehouse) return;
    try {
      const numericId = warehouse.numericId || warehouse.id;
      if (!numericId) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-2">
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-center sm:justify-start gap-3">
            <div className={`p-2.5 sm:p-3 rounded-xl flex-shrink-0 ${isDark ? 'bg-[#C2D642]/10' : 'bg-[#C2D642]/5'}`}>
              <Warehouse className="w-5 h-5 sm:w-6 sm:h-6 text-[#C2D642]" />
            </div>
            <h1 className={`text-xl sm:text-2xl font-black tracking-tight ${textPrimary}`}>Warehouses</h1>
          </div>
        ...
        <div className="flex flex-wrap items-center justify-center sm:justify-end gap-2">
          ...
          <button 
            ...
            disabled={!selectedProjectId}
            className={`flex items-center gap-1 sm:gap-2 px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-bold transition-all ${isDark ? 'bg-[#C2D642] hover:bg-[#C2D642] text-white' : 'bg-[#C2D642] hover:bg-[#C2D642] text-white'} shadow-md disabled:opacity-50 disabled:cursor-not-allowed`}
          >
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className={`${cardClass} rounded-xl border w-full max-w-lg p-6 shadow-xl`}>
            ...
            <p className={`text-sm ${textSecondary} mb-6`}>
              Are you sure you want to delete warehouse <span className={`font-bold ${textPrimary}`}>{deleteConfirmWarehouse.name}</span>? This action cannot be undone.
            </p>
            <div className="flex items-center justify-end gap-3">
              <button
                onClick={() => setDeleteConfirmWarehouse(null)}
              ...
              <button
                onClick={() => handleDeleteWarehouse(deleteConfirmWarehouse)}
