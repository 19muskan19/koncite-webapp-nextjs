...
const Vendors: React.FC<VendorsProps> = ({ theme }) => {
  ...
  const handleDeleteAllVendors = async () => {
    if (vendors.length === 0) {
      toast.showWarning('No vendors to delete.');
    ...
    try {
      for (const vendor of vendors) {
        const deleteId = String(vendor.numericId ?? vendor.uuid ?? vendor.id);
        try {
          await masterDataAPI.deleteVendor(deleteId);
          deleted++;
        } catch (err) {
          failed++;
      ...
      await fetchVendors();
      if (deleted > 0) {
        toast.showSuccess(`Deleted ${deleted} vendor${deleted !== 1 ? 's' : ''}.`);
      }
      if (failed > 0) {
        toast.showError(`Failed to delete ${failed} vendor${failed !== 1 ? 's' : ''}.`);
      }
    } finally {
  ...
  const handleToggleStatus = async (vendor: Vendor) => {
    if (togglingVendorId === vendor.id || isLoadingVendors) return;

    try {
      setTogglingVendorId(vendor.id);

      const currentStatus = vendor.status || (vendor.is_active === 1 ? 'Active' : 'Inactive');
      const newStatus = currentStatus === 'Active' ? 'Inactive' : 'Active';
      const isActive = (newStatus === 'Active' ? 1 : 0) as 0 | 1;

      // Backend updateStatus($uuid) uses uuidtoid() - must pass UUID
      const vendorUuid = vendor.uuid || vendor.id;
      if (!vendorUuid) {
        toast.showError('Vendor identifier not found');
        return;
      }
      ...
      setVendors(prev =>
        prev.map(v =>
          v.id === vendor.id ? { ...v, status: newStatus, is_active: isActive } : v
        )
      );

      await masterDataAPI.updateVendorStatus(vendorUuid, isActive);

      toast.showSuccess(newStatus === 'Inactive' ? 'Vendor deactivated successfully' : 'Vendor activated successfully');

      if (newStatus === 'Inactive') {
        setOpenDropdownId(prev => (prev === vendor.id ? null : prev));
      }
    } catch (error: any) {
      // Revert optimistic update on error
      setVendors(prev =>
        prev.map(v =>
          v.id === vendor.id ? { ...v, status: vendor.status, is_active: vendor.is_active } : v
        )
      );
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-2">
        ...
        <div className="flex flex-wrap items-center justify-center sm:justify-end gap-2">
          ...
          <button 
            ...
            disabled={vendors.length === 0 || isDeletingAll}
            className={`flex items-center gap-1 sm:gap-2 px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-bold transition-all ${
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <button
                        ...
                        disabled={isLoadingVendors || togglingVendorId === row.id}
                        ...
                        } ${(isLoadingVendors || togglingVendorId === row.id) ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="relative">
                        <button 
                          ...
                          title={row.status === 'Inactive' ? 'Activate vendor to enable actions' : 'Actions'}
                        >
                          <MoreVertical className={`w-4 h-4 ${textSecondary}`} />
                        </button>
                        {openDropdownId === row.id && row.status !== 'Inactive' && (
                          <div className={`dropdown-menu absolute right-0 w-32 rounded-lg border shadow-xl z-[100] ${idx === filteredVendors.length - 1 ? 'bottom-full mb-1' : 'top-full mt-1'} ${isDark ? 'bg-dropdown-panel border-slate-700' : 'bg-white border-slate-200'}`}>
                            <div className="py-1">
                              <button
                                onClick={() => {
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className={`${cardClass} rounded-xl border w-full max-w-lg p-6 shadow-xl`}>
            ...
            <p className={`text-sm ${textSecondary} mb-6`}>
              Are you sure you want to delete all <span className={`font-bold ${textPrimary}`}>{vendors.length}</span> vendor{vendors.length !== 1 ? 's' : ''}? This action cannot be undone.
            </p>
