...
const Activities: React.FC<ActivitiesProps> = ({ theme }) => {
  ...
  const handleDeleteAllActivities = async () => {
    if (activities.length === 0) {
      toast.showWarning('No activities to delete.');
    ...
    try {
      for (const activity of activities) {
        ...
        try {
          ...
          failed++;
          console.error('Failed to delete activity:', activity.name || activity.activities, err);
        }
      }
      await fetchActivities();
      if (deleted > 0) {
        toast.showSuccess(`Deleted ${deleted} activit${deleted !== 1 ? 'ies' : 'y'}.`);
      }
      if (failed > 0) {
        toast.showError(`Failed to delete ${failed} activit${failed !== 1 ? 'ies' : 'y'}.`);
      }
    } finally {
