...
const ActivityBulkUploadModal: React.FC<ActivityBulkUploadModalProps> = ({
  ...
  const canOperate = !!bulkProjectId && !!bulkSubprojectId;
  ...
  const handleExport = async () => {
    ...
    try {
      const project = projects.find((p) => p.uuid === bulkProjectId || String(p.id) === bulkProjectId);
      const subproject = subprojects.find((s) => s.uuid === bulkSubprojectId || String(s.id) === bulkSubprojectId);
      const projectIdNum = project?.id ?? bulkProjectId;
      const subprojectIdNum = subproject?.id ?? bulkSubprojectId;
      ...
      <div className={`${bgPrimary} rounded-xl border ${cardClass} w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl`}>
        <div className="flex items-center justify-between p-6 border-b border-inherit">
          <div>
            ...
            <p className={`text-sm ${textSecondary} mt-1`}>
              Select project and subproject, then export or import activities data
            </p>
          </div>
          <button
        ...
        <div className="p-6 space-y-6 overflow-y-auto flex-1">
          ...
          <div className={`rounded-lg border p-4 ${isDark ? 'bg-slate-800/50 border-slate-600' : 'bg-slate-50 border-slate-200'}`}>
            <h3 className={`text-sm font-bold ${textPrimary} mb-3`}>Select project and subproject</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className={`block text-xs font-bold mb-1.5 ${textSecondary}`}>Subproject</label>
          ...
          {!canOperate && (
            <p className={`text-xs ${textSecondary} italic`}>
              Select both project and subproject to enable export and import.
            </p>
          )}
