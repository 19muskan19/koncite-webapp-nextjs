...
const UserProfileModal: React.FC<UserProfileModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (hasCountryId && !hasCountryCode && countries.length > 0) {
      ...
      if (phoneCode) {
        setFormData((prev) => prev.countryCode ? prev : { ...prev, countryCode: phoneCode });
      }
    }
  ...
  const fetchCountryCodes = async () => {
    ...
    try {
      ...
      setCountryCodes(Array.from(byCode.values()).sort((a, b) => {
                              ...
                              .map((cc) => (
                                <button
                                  ...
                                  className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-left ${
                                    formData.countryCode === cc.dialCode ? (isDark ? 'bg-[#C2D642]/20' : 'bg-[#C2D642]/10') : isDark ? 'hover:bg-slate-700' : 'hover:bg-slate-100'
                                  }`}
                                >
            ...
            <div className="relative">
              ...
              >
                {formData.countryCode && countryCodes.length > 0
                  ? (countryCodes.find((c) => c.dialCode === formData.countryCode)?.name || (formData.country && countries.length > 0 ? countries.find((c) => String(c.id) === formData.country)?.name : null) || 'Select a country code above')
