...
import { financeAPI, type DashboardStats, type TimeseriesPoint } from '@/services/financeApi';
...
export default function DashboardTab({ isDark }: DashboardTabProps) {
  ...
  useEffect(() => {
    const refresh = () => {
      ...
      financeAPI.getRevenueVsExpenses().then(setSeries);
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-2 sm:gap-3">
        ...
        <StatCard icon={<FolderKanban className="w-5 h-5" />} title="Active Projects" value={stats.activeProjects} variant="amber" isDark={isDark} valueFormat="number" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className={cn('rounded-xl border p-6 transition-all hover:border-[#C2D642]/30', isDark ? 'card-dark' : 'card-light')}>
          <h3 className={cn('text-xs font-black uppercase tracking-widest mb-4', isDark ? 'text-slate-400' : 'text-slate-600')}>Income vs expenses (daily)</h3>
          <div className="h-64">
            {series.length === 0 ? (
              <p className={cn('text-sm font-semibold flex items-center justify-center h-full', isDark ? 'text-slate-500' : 'text-slate-500')}>
                No timeseries data for this range yet
              </p>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={series.slice(-14)}>
                  ...
                  <Tooltip
                    ...
                    contentStyle={{ backgroundColor: isDark ? '#1e293b' : '#fff', border: `1px solid ${isDark ? '#334155' : '#e2e8f0'}` }}
                  />
        ...
        <div className={cn('rounded-xl border p-6 transition-all hover:border-[#C2D642]/30', isDark ? 'card-dark' : 'card-light')}>
          ...
          <div className="flex flex-col items-center gap-4">
            {expenseDist.length === 0 ? (
              <p className={cn('text-sm font-semibold py-12', isDark ? 'text-slate-500' : 'text-slate-500')}>No expense categories yet</p>
            ) : (
              <>
