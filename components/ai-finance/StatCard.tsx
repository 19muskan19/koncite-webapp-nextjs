...
interface StatCardProps {
  ...
  isDark?: boolean;
  className?: string;
  /** Use plain number for counts (e.g. active projects). */
  valueFormat?: 'currency' | 'number';
}

export default function StatCard({ icon, title, value, trend, variant = 'emerald', isDark = false, className, valueFormat = 'currency' }: StatCardProps) {
  ...
  return (
    <div className={cn('rounded-lg border p-3 transition-all duration-200 overflow-hidden hover:shadow-md hover:border-[#C2D642]/40', cardClass, className)}>
      <div className="flex flex-col">
        ...
        <p className={cn('mt-0.5 text-base sm:text-lg font-black', textPrimary)}>
          {valueFormat === 'number' ? value.toLocaleString('en-IN') : formatCurrency(value)}
        </p>
        {trend != null && (
