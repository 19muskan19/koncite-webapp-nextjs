...
export default function GoodsReturnFlow({
      ...
      goodsReturnAPI
        ...
        .then((data) => {
          ...
          if (projId) {
            ...
            setReturnDate(data?.date ?? data?.name ?? new Date().toISOString().split('T')[0]);
  ...
  const handleStoresNext = async () => {
    ...
    if (storeNumericIds.length === 0) {
      toast.showWarning('Invalid store selection.');
      return;
    }
    setIsCreatingHeader(true);
    try {
      const name = returnDate || new Date().toISOString().split('T')[0];
      const payload = {
        name,
        projects_id: pId,
  ...
  const handleAssetReturnNext = async () => {
    ...
    const returnNo = returnHeader.return_no ?? returnHeader.name ?? returnDate;
    const pId = projectIdForApi();
    const storeIdsArr = Array.from(selectedStoreIds);
    const storeNumericIds = storeIdsArr
      .map((sid) => {
    ...
    if (!returnFromId) {
      toast.showWarning('Return reason (Return from) is required.');
      return;
    }
    ...
    try {
      ...
      const goodsListRaw = Array.isArray(addResult) ? addResult : addResult?.data ?? addResult?.return_goods ?? [];
      ...
      if (goodsList.length > 0) {
        detailItems = goodsList.map((g: any) => ({
          inv_return_goods_id: g.id ?? g.inv_return_goods_id ?? invReturnId,
        ...
        detailItems = materialNumericIds.map((mid) => {
          ...
          return {
            inv_return_goods_id: invReturnId,
            materials_id: mid,
  ...
  const handleDetailsNext = async () => {
    ...
    const storeNumericIds = Array.from(selectedStoreIds)
      ...
      .filter((x): x is string | number => x != null);
    const invReturnGoodsId = returnGoodsList?.[0]?.id ?? returnHeader?.id ?? details?.[0]?.inv_return_goods_id;
    setIsSubmitting(true);
    try {
      ...
      await goodsReturnAPI.addReturnDetails(payload);
      setStep('success');
      const returnHeaderId = returnHeader?.id ?? returnHeader?.inv_returns_id ?? returnHeader?.uuid ?? invReturnGoodsId ?? editReturnId;
      const { pdf_url, name } = await goodsReturnAPI.generatePdf(returnHeaderId!);
      setPdfInfo({ url: pdf_url, name: name ?? `Return-${returnDate}.pdf` });
    } catch (e: any) {
  ...
  const updateDetailQty = (materialsId: string, qty: number | string) => {
    setDetails((prev) =>
      prev.map((d) => (String(d.materials_id) === materialsId ? { ...d, return_qty: qty } : d))
    );
  };
  ...
  const updateDetailActivity = (materialsId: string, activityId: string | number, activityName: string) => {
    setDetails((prev) =>
      prev.map((d) =>
        String(d.materials_id) === materialsId ? { ...d, activities_id: activityId, activityName } : d
      )
    );
  };
          ...
          <div className={`rounded-xl border p-6 ${cardClass}`}>
            ...
            <div className="flex justify-end">
              <button
                ...
                disabled={isCreatingHeader || selectedStoreIds.size === 0}
                className={`flex items-center gap-2 px-6 py-2 rounded-lg font-bold ${isCreatingHeader || selectedStoreIds.size === 0 ? 'opacity-50 cursor-not-allowed' : ''} bg-[#6B8E23] text-white hover:bg-[#5a7a1e]`}
              >
          ...
          <div className={`rounded-xl border p-6 ${cardClass}`}>
            ...
            <div className={`p-4 rounded-lg mb-4 space-y-2 ${isDark ? 'bg-slate-800/50' : 'bg-slate-50'}`}>
              ...
              <div>
                ...
                <p className={textPrimary}>{returnHeader?.return_no ?? returnHeader?.name ?? returnDate}</p>
              </div>
            </div>
            ...
            <div className="flex justify-end">
              <button
                ...
                disabled={isSubmitting || selectedMaterialIds.size === 0}
                className={`flex items-center gap-2 px-6 py-2 rounded-lg font-bold ${isSubmitting || selectedMaterialIds.size === 0 ? 'opacity-50 cursor-not-allowed' : ''} bg-[#6B8E23] text-white hover:bg-[#5a7a1e]`}
              >
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      className={`w-full flex items-center justify-between p-4 text-left ${isDark ? 'hover:bg-slate-800/50' : 'hover:bg-slate-50'}`}
                    >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div>
                            ...
                            <input
                              ...
                              min={0}
                              value={d.return_qty}
                              onChange={(e) => updateDetailQty(key, e.target.value)}
                              className={`w-full px-4 py-2 rounded-lg border ${isDark ? 'bg-slate-800 border-slate-600' : 'bg-white border-slate-200'}`}
                            />
                          </div>
                          <div>
                          ...
                          <div className="sm:col-span-2">
                            ...
                            <button
                              type="button"
                              onClick={() => setShowTagActivityModal(key)}
                            ...
                            >
                              ...
                              {d.activityName || 'Select activity'}
                            </button>
                          </div>
                        </div>
                      </div>
          ...
          <div className={`rounded-xl border p-8 ${cardClass} text-center`}>
            ...
            {pdfInfo?.url && (
              <div className={`p-4 rounded-xl border ${isDark ? 'border-slate-600' : 'border-slate-200'}`}>
                ...
                <p className={`font-mono text-sm mb-3 ${textPrimary}`}>{pdfInfo.name || 'Return.pdf'}</p>
                <div className="flex flex-wrap justify-center gap-2">
                  <button
                    onClick={() => window.open(pdfInfo.url, '_blank')}
                    className="flex items-center gap-2 px-4 py-2 rounded-lg border border-[#6B8E23] text-[#6B8E23] hover:bg-[#6B8E23]/10"
                  >
                    <ExternalLink className="w-4 h-4" />
                    View
                  </button>
                  <button
                    onClick={() => navigator.share?.({ url: pdfInfo.url, title: pdfInfo.name })}
                    className="flex items-center gap-2 px-4 py-2 rounded-lg border border-[#6B8E23] text-[#6B8E23] hover:bg-[#6B8E23]/10"
                  >
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className={`${cardClass} rounded-xl p-6 max-w-md w-full`}>
              ...
              <ul className={`text-sm space-y-2 ${textSecondary}`}>
                <li>• Project and Store: mandatory</li>
                <li>• Materials/Assets: mandatory</li>
