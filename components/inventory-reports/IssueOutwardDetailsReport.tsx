...
interface IssueOutwardDetailsReportProps {
  theme: ThemeType;
}
...
const IssueOutwardDetailsReport: React.FC<IssueOutwardDetailsReportProps> = ({ theme }) => {
  ...
  const loadReportData = useCallback(async () => {
    ...
    try {
      ...
      toast.showError(err?.message || 'Failed to load Issue Outward Details report');
      setTableData([]);
    } finally {
      setIsLoading(false);
    }
  }, [selectedProject, selectedStore, selectedIssueTo, fromDate, toDate, searchQuery, activeTab, projects, issueTypes, toast]);

  useEffect(() => {
    if (selectedProject && selectedStore && fromDate && toDate) loadReportData();
  }, [selectedProject, selectedStore, selectedIssueTo, fromDate, toDate, searchQuery, activeTab, loadReportData]);
  ...
  const handleExport = (format: string) => {
    ...
    if (format === 'Copy') {
      ...
      a.download = 'issue-outward-details-report.csv';
      a.click();
      ...
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = 'issue-outward-details-report.xlsx';
      a.click();
      URL.revokeObjectURL(a.href);
      toast.showSuccess('Downloaded');
    } else if (format === 'PDF') {
    ...
    <div className="space-y-6 p-2 sm:p-0">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className={`text-2xl font-black tracking-tight ${textPrimary}`}>Issue (Outward) Details Report</h1>
            <p className={`text-[11px] font-bold opacity-50 uppercase tracking-widest mt-1 ${textSecondary}`}>
              View issue outward details by material or machines/assets
            </p>
          </div>
        </div>
      </div>
...
export default IssueOutwardDetailsReport;
