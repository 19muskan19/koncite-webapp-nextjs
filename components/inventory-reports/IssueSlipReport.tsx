...
interface IssueSlipReportProps {
  theme: ThemeType;
}
...
const IssueSlipReport: React.FC<IssueSlipReportProps> = ({ theme }) => {
  ...
  const [selectedIssueTo, setSelectedIssueTo] = useState<string>('');
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const arr = await goodsIssueAPI.getIssueTypeList();
        const list = Array.isArray(arr) ? arr : ((arr as { data?: any[] })?.data ?? []);
        setIssueTypes(list.map((e: any) => ({ id: e.id ?? e.uuid, name: e.name ?? e.issue_type ?? '' })));
      } catch {
        setIssueTypes([]);
      }
    };
  ...
  const loadReportData = useCallback(async () => {
    ...
    try {
      ...
      const dateStr = selectDate.length >= 10 ? selectDate.slice(0, 10) : selectDate;
      ...
      try {
        ...
        for (const item of arr) {
          ...
          const d = item?.date ?? item?.issue_date ?? '-';
          const dateVal = typeof d === 'string' && d.length >= 10 ? d.slice(0, 10) : (d || '-');
          rows.push({
            id: `${item?.id ?? item?.issue_no ?? Math.random()}-${rows.length}`,
            issueNo,
            date: dateVal,
            code,
      ...
      if (rows.length === 0) {
        ...
        const filtered = issues.filter((inv: any) => {
          ...
          if (selectedIssueTo) {
            const issueToId = inv?.issue_to ?? inv?.entry_type ?? inv?.type;
            if (String(issueToId) !== String(selectedIssueTo)) return false;
          }
          if (dateStr) {
          ...
          if (searchQuery.trim()) {
            ...
            const issueNo = (inv?.issue_no ?? inv?.name ?? inv?.id ?? '').toString().toLowerCase();
            if (!issueNo.includes(q)) return false;
          }
          return true;
        });

        const issueTypeMap = Object.fromEntries(issueTypes.map((t) => [String(t.id), t.name]));

        for (const inv of filtered) {
          try {
            const invId = inv?.id ?? inv?.uuid ?? inv?.inv_issues_id;
            const editData = await goodsIssueAPI.edit(invId);
            const details = editData?.details ?? editData?.issue_details ?? editData?.issue_goods ?? [];
            const list = Array.isArray(details) ? details : [];
            const issueNo = inv?.issue_no ?? inv?.name ?? inv?.id ?? '-';
            const invDate = inv?.date ?? inv?.created_at ?? '-';
            const dateVal = typeof invDate === 'string' && invDate.length >= 10 ? invDate.slice(0, 10) : (invDate || '-');
            const issueToName = issueTypeMap[String(editData?.issue_to ?? inv?.issue_to ?? '')] ?? editData?.issue_type?.name ?? '-';
            const issueByName = editData?.user?.name ?? editData?.created_by ?? inv?.user?.name ?? '-';
            ...
            for (const d of list) {
              ...
              const issueQty = Number(d?.issue_qty ?? d?.qty ?? 0);
              const activities = d?.activities?.name ?? d?.activityName ?? d?.activity_name ?? '-';
              rows.push({
      ...
      toast.showError(err?.message || 'Failed to load Issue Slip report');
      setTableData([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const filteredAndSorted = useMemo(() => {
    let out = tableData.filter(
      (r) =>
        tableSearch.trim() === '' ||
        [r.issueNo, r.code, r.materialsMachine, r.specification, r.unit, r.activities, r.issueBy, r.issueTo].some((v) =>
  ...
  const headers = ['Issue No', 'Date', 'Code', 'Materials/Machine', 'Specification', 'Unit', 'Issue Qty', 'Activities', 'Issue By', 'Issue To'];
  const handleExport = (format: string) => {
    ...
    if (format === 'Copy') {
      ...
      a.download = 'issue-slip-report.csv';
      a.click();
      ...
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = 'issue-slip-report.xlsx';
      a.click();
      URL.revokeObjectURL(a.href);
      toast.showSuccess('Downloaded');
    } else if (format === 'PDF') {
      ...
      doc.setFontSize(16);
      doc.text(`Issue Slip Report - ${activeTab === 'materials' ? 'Material' : 'Machines/Assets'}`, 14, 15);
      doc.setFontSize(10);
      ...
      doc.save('issue-slip-report.pdf');
      toast.showSuccess('Downloaded');
    } else if (format === 'Print') {
      const printContent = `
<!DOCTYPE html><html><head><title>Issue Slip Report</title>
...
<h1>Issue Slip Report - ${activeTab === 'materials' ? 'Material' : 'Machines/Assets'}</h1>
...
<tbody>${filteredAndSorted.map((r) => `<tr><td>${r.issueNo}</td><td>${r.date}</td><td>${r.code}</td><td>${r.materialsMachine}</td><td>${r.specification}</td><td>${r.unit}</td><td>${formatNum(r.issueQty)}</td><td>${r.activities}</td><td>${r.issueBy}</td><td>${r.issueTo}</td></tr>`).join('')}</tbody></table>
  ...
  const colKeys = ['issueNo', 'date', 'code', 'materialsMachine', 'specification', 'unit', 'issueQty', 'activities', 'issueBy', 'issueTo'];
  const rightAlignKeys = ['issueQty'];
    ...
    <div className="space-y-6 p-2 sm:p-0">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className={`text-2xl font-black tracking-tight ${textPrimary}`}>Issue Slip Report</h1>
            <p className={`text-[11px] font-bold opacity-50 uppercase tracking-widest mt-1 ${textSecondary}`}>
              View issue slip details by material or machines/assets
            </p>
          </div>
        </div>
      </div>
      ...
      <div className={`rounded-xl border ${cardClass} p-4`}>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          ...
          <div>
            <label className={`block text-sm font-bold mb-2 ${textPrimary}`}>Issue To</label>
            <select
              value={selectedIssueTo}
              onChange={(e) => setSelectedIssueTo(e.target.value)}
            ...
            >
              ...
              {issueTypes.map((e) => <option key={String(e.id)} value={String(e.id)}>{e.name}</option>)}
            </select>
          </div>
          <div>
                  ...
                  {colKeys.map((k) => (
                    <th key={k} className={`px-4 py-3 font-bold ${textPrimary} cursor-pointer ${rightAlignKeys.includes(k) ? 'text-right' : 'text-left'}`} onClick={() => handleSort(k)}>
                      <span className="flex items-center gap-2">
                        {k === 'issueNo' ? 'Issue No' : k === 'date' ? 'Date' : k === 'code' ? 'Code' : k === 'materialsMachine' ? 'Materials/Machine' : k === 'specification' ? 'Specification' : k === 'unit' ? 'Unit' : k === 'issueQty' ? 'Issue Qty' : k === 'activities' ? 'Activities' : k === 'issueBy' ? 'Issue By' : 'Issue To'}
                        {getSortIcon(k)}
                      </span>
                    </th>
                  ))}
              ...
              <tbody>
                {paginated.map((row) => (
                  <tr key={row.id} className={`border-t border-inherit ${isDark ? 'hover:bg-slate-800/30' : 'hover:bg-slate-50'}`}>
                    <td className={`px-4 py-3 ${textPrimary}`}>{row.issueNo}</td>
                    ...
                    <td className={`px-4 py-3 ${textSecondary}`}>{row.activities}</td>
                    <td className={`px-4 py-3 ${textPrimary}`}>{row.issueBy}</td>
                ...
                {!isLoading && paginated.length === 0 && (
                  <tr>
                    <td colSpan={10} className={`px-4 py-12 text-center ${textSecondary}`}>
                      No data available. Select a project and ensure there are issue slips in the date range.
                    </td>
                  </tr>
                )}
              </tbody>
...
export default IssueSlipReport;
