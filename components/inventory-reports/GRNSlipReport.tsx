...
const GRNSlipReport: React.FC<GRNSlipReportProps> = ({ theme }) => {
  ...
  const mergedReportMeta = useMemo((): InventoryReportMeta | null => {
    ...
    const selectedDate =
      api?.selectedDate && (api.selectedDate.from || api.selectedDate.to || api.selectedDate.date)
        ? api.selectedDate
        : fromDate.length >= 10
          ? { from: fromDate.slice(0, 10), to: fromDate.slice(0, 10) }
          : api?.selectedDate;
  ...
  }, [reportMeta, userCompany, projects, selectedProject, fromDate]);
  ...
  const handleExport = (format: string) => {
    ...
    if (format === 'Copy') {
      ...
      a.download = 'grn-slip-report.csv';
      a.click();
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'GRN Slip');
      const excelBuffer = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
      const blob = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = 'grn-slip-report.xlsx';
      a.click();
      URL.revokeObjectURL(a.href);
      toast.showSuccess('Downloaded');
    } else if (format === 'PDF') {
      void (async () => {
        try {
          ...
          const reportTitle = `GRN (MRN) Slip Report - ${activeTab === 'materials' ? 'Material' : 'Machines/Assets'}`;
