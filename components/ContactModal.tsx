...
const ContactModal: React.FC<ContactModalProps> = ({ isOpen, onClose }) => {
  ...
  const fetchCountries = async () => {
    ...
    try {
      ...
      setCountryCodes(sortCountryCodes(transformedCountries));
    } catch (error) {
  ...
  const selectedCountry = findCountryByDialCode(countryCodes, formData.countryCode) || 
    findCountryByDialCode(countryCodes, '91') || // Default to India
    countryCodes[0] || 
    { code: 'IN', dialCode: '91', name: 'India', flag: getFlagUrl('IN') };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className={`${cardClass} rounded-xl shadow-2xl w-full max-w-[min(92vw,1024px)] mx-4 relative border ${borderClass} max-h-[90vh] overflow-hidden flex flex-col`}>
        <button
          onClick={onClose}
